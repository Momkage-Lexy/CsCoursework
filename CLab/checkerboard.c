...
void set_image_data(png_bytep * data, int w, int h, int square) {
    // Loop through every pixel and set the color
    for (int y = 0; y < h; y++) {
        for (int x = 0; x < w; x++) {
            // Black square
            int black = ((x / square) % 2 == (y / square) % 2);

            // Check if square is black or white
            png_byte color = black ? 0x00 : 0xFF; 
...
int main(int argc, char **argv)
{
    ...
    set_image_data(row_ptrs, width, height,square);
