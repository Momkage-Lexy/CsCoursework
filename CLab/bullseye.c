...
void set_image_data(png_bytep * data, int w, int h, int radius) {
    // Calculate the center 
    int cx = w / 2;
    int cy = h / 2;

    // Calculate the maximum distance from the center to the edge of the circle
    float max_radius = fminf(cx, cy);

    // Loop through every pixel and set the color
    for (int y = 0; y < h; y++) {
        for (int x = 0; x < w; x++) {

            // Set x/y values from center
            float dx = x - cx;
            float dy = y - cy;

            // Set Euclidean distance from the current pixel to the center
            float distance = sqrtf(dx * dx + dy * dy);

            // Check if the distance is within the circle radius
            if (distance <= max_radius) {
                // Normalize the distance to be between 0 (center) and 1 (maximum distance)
                float normalized_distance = distance / max_radius;

                // Get the corresponding RGB color using the normalized distance
                struct rgb color = value2color(normalized_distance, 0.0, 1.0);

                // Set the RGB values for each pixel in the row
                data[y][4 * x] = color.red;
                data[y][4 * x + 1] = color.green;
                data[y][4 * x + 2] = color.blue;
                data[y][4 * x + 3] = 255;  
            } else {
                // Set pixels outside the circle to be fully transparent
                data[y][4 * x] = 0;      
                data[y][4 * x + 1] = 0;  
                data[y][4 * x + 2] = 0;  
                data[y][4 * x + 3] = 0;   
...
int main(int argc, char **argv)
{
    ...
    {
        row_ptrs[j] = (png_bytep)malloc(sizeof(png_byte) * 4 * width);
    ...
    set_image_data(row_ptrs, width, height, radius);
